...
FastAPI 后端保留现有 RAG 接口；前端使用 Vue 3、TypeScript 和 Vite，生产构建输出到 `rag_project/web` 并由 FastAPI 提供静态资源。
...
- 文档列表已合并 29 篇只读学习笔记与上传资料；PDF/TXT/Markdown 上传、删除和重新解析已接入真实后端，并会触发向量索引更新。
- 会话历史、用户登录、流式生成和回答反馈尚无持久化后端；前端只能以明确标注的本地演示数据呈现。
- 保持已有 API 行为和自动化测试兼容。
