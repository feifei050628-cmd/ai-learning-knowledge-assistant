...
const documentCategory = ref<"all" | "reference" | "learning_note">("all");
...
const filteredDocuments = computed(() => documents.value.filter((item) => {
  const matchesName = item.name.toLowerCase().includes(documentSearch.value.toLowerCase());
  const matchesCategory = documentCategory.value === "all" || item.category === documentCategory.value;
...
const learningNoteCount = computed(() => documents.value.filter((item) => item.category === "learning_note").length);
  ...
  <div class="app-shell">
    ...
    <main id="main-content" :class="['main', `view-${view}`, { 'evidence-collapsed': !evidenceOpen }]">
      ...
      <section v-else-if="view === 'documents'" class="management-page">
        ...
        <div class="stats-strip"><div><span>文档总数</span><strong>{{ documents.length }}</strong></div><div><span>学习笔记</span><strong>{{ learningNoteCount }}</strong></div><div><span>可用文档</span><strong>{{ readyCount }}</strong></div><div><span>可检索片段</span><strong>{{ totalChunks }}</strong></div></div>
        <div class="table-toolbar"><label><Search :size="17" /><input v-model="documentSearch" placeholder="搜索文件名" /></label><div class="document-filters" aria-label="文档分类"><button :class="{ active: documentCategory === 'all' }" @click="documentCategory = 'all'">全部</button><button :class="{ active: documentCategory === 'learning_note' }" @click="documentCategory = 'learning_note'">学习笔记</button><button :class="{ active: documentCategory === 'reference' }" @click="documentCategory = 'reference'">上传资料</button></div><span>{{ filteredDocuments.length }} 个文档</span></div>
        <div class="document-table" role="table" aria-label="文档列表"><div class="table-row table-header" role="row"><span>文件</span><span>类型</span><span>大小</span><span>更新时间</span><span>处理状态</span><span>操作</span></div><div v-if="documentsLoading" class="table-empty"><LoaderCircle class="spin" :size="25" /><p>正在读取真实文档列表…</p></div><div v-for="document in filteredDocuments" v-else :key="document.id" class="table-row" role="row"><span class="file-cell"><FileText :size="19" /><span><b :title="document.name">{{ document.name }}</b><small>{{ document.category === 'learning_note' ? '学习笔记 · ' : '上传资料 · ' }}{{ document.error || (document.chunks ? `${document.chunks} 个片段` : '尚无可检索片段') }}</small></span></span><span><em class="file-type">{{ document.type }}</em></span><span>{{ formatBytes(document.size_bytes) }}</span><span>{{ formatDate(document.updated_at) }}</span><span><em :class="['status-label', activeDocumentOperation === document.id ? 'processing' : document.status]"><LoaderCircle v-if="activeDocumentOperation === document.id" :size="13" /><CircleCheck v-else-if="document.status === 'ready'" :size="13" /><AlertCircle v-else :size="13" />{{ activeDocumentOperation === document.id ? '处理中' : document.status === 'ready' ? '可用' : '失败' }}</em></span><span v-if="document.read_only" class="read-only-label" title="学习笔记由项目根目录维护">只读</span><span v-else class="row-actions"><button title="重新解析" :disabled="Boolean(activeDocumentOperation)" @click="reparse(document)"><RefreshCw :size="16" /></button><button title="删除" :disabled="Boolean(activeDocumentOperation)" @click="confirmDelete = document"><Trash2 :size="16" /></button></span></div><div v-if="!documentsLoading && !filteredDocuments.length" class="table-empty"><Search :size="25" /><p>{{ documentSearch || documentCategory !== 'all' ? '没有找到匹配的文档' : '知识库中还没有文档' }}</p><small v-if="!documentSearch && documentCategory === 'all'">上传 PDF、TXT 或 Markdown 后即可开始检索。</small></div></div>
      </section>
      ...
      <section v-else-if="view === 'knowledge'" class="management-page">
        ...
        <div class="knowledge-grid"><article><Database :size="20" /><span>知识源</span><strong>{{ documents.length }}</strong><small>{{ learningNoteCount }} 篇学习笔记 · {{ documents.length - learningNoteCount }} 份上传资料</small></article><article><FileText :size="20" /><span>文本片段</span><strong>{{ totalChunks }}</strong><small>最大 500 字符 · 80 字符重叠</small></article><article><Search :size="20" /><span>默认检索参数</span><strong>Top {{ topK }}</strong><small>相关性门槛 {{ minSimilarity.toFixed(2) }}</small></article></div>
      </section>
